...
const quoteRequest = {
  ...
  properties: {
    ...
    retrieval: { type: "string", enum: ["scrape", "browse"], description: 'Retrieval the host performs. Default "scrape" ($0.01). "browse" is an explicit opt-in to reader.outbid.sh/browse ($0.06) — scrape never auto-upgrades. A scrape that needs a browser is a free 422 needs_browser.', example: "scrape" },
...
const badRequest = out('Malformed request — never billed. reason: "bad_json" (unparseable body); "bad_extract" (extract is not the shape above: wrong dialect, empty, over the key-count or key-length bound, or a typename outside number|string); "bad_assertion" (assertion is neither null nor a string within maxLength); "bad_retrieval" (retrieval is neither scrape nor browse). bad_extract, bad_assertion, and bad_retrieval also carry expected (the accepted shape) and example (a value to copy).', {
  type: "object",
  required: ["reason"],
  properties: {
    reason: { type: "string", enum: ["bad_json", "bad_extract", "bad_assertion", "bad_retrieval"] },
